...
python 容器
    ...
    docker run --name env-3.8.6 -itd -p 10022:22 --privileged=true python:3.8.6 /bin/bash
...
docker run -itd --name env-3.8.7 -p 10028:22 -p 3306:3306 -p 6379:6379 -p 27017:27017 -p 9200:9200 -p 9300:9300 py-3.8.7:v1 /bin/bash

使用container网络构建环境：
    1.先docker run 一个Python环境的容器(py-env),需要映射出docker-compose所有服务需要的端口
    2.docker-compose network_mode:"py-env"
    
    Python环境容器和docker-compose各服务容器共享一个网络栈，可以使用127.0.0.1互相访问
